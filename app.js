document.addEventListener('DOMContentLoaded', () => {
    ...
    function copyTextToClipboard() {
        ...
        navigator.clipboard.writeText(DOM.resultEl.value)
            .then(() => {
    ...
    function setupCursor() {
        ...
        if (!DOM.cursor) {
            console.warn('Cursor element missing');
            return;
        }
        ...
        const tryPlay = () =>
            DOM.cursor.play()
                .then(() => console.log('cursor playing'))
                .catch(() => {
                    DOM.cursor.style.display = 'none';
                    document.body.style.cursor = 'auto';
                });

        if (DOM.cursor.readyState >= 4) tryPlay();
        else DOM.cursor.addEventListener('canplaythrough', tryPlay, { once: true });

        DOM.cursor.addEventListener('error', () => {
            DOM.cursor.style.display = 'none';
            document.body.style.cursor = 'auto';
        });
        ...
        document.querySelectorAll('button, a, input, textarea').forEach(el => {
            el.addEventListener('mouseenter', () => DOM.cursor.pause());
            el.addEventListener('mouseleave', () => DOM.cursor.play().catch(() => {}));
        });
    }
